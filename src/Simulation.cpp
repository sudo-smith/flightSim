...
namespace Simulation {
	...
	static GLint * s_flagTex_nullData;
	static GLfloat * s_geoTex_nullData;
	static GLfloat * s_outlineTex_nullData;
	static GLubyte * s_sideTex_nullData;
	...
	static void computeReset(int swap) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_R32UI, k_width, k_height, 0, GL_RED_INTEGER, GL_UNSIGNED_INT, s_flagTex_nullData);
	...
	bool step() {
		if (s_currentSlice == 0) {
			//clear textures 4.3 way
			glBindTexture(GL_TEXTURE_2D, s_flagTex);
			glTexImage2D(GL_TEXTURE_2D, 0, GL_R32UI, k_width, k_height * 2, 0, GL_RED_INTEGER, GL_INT, s_flagTex_nullData);

			glBindTexture(GL_TEXTURE_2D, s_geoTex);
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, k_estimateMaxGeoPixels, k_extimateMaxGeoPixelsRows, 0, GL_RGBA, GL_FLOAT, s_geoTex_nullData);

			glBindTexture(GL_TEXTURE_2D, s_outlineTex);
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, k_estimateMaxOutlinePixels, k_estimateMaxOutlinePixelsRows, 0, GL_RGBA, GL_FLOAT, s_outlineTex_nullData);

			glBindTexture(GL_TEXTURE_2D, s_sideTex);
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, k_width, k_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, s_sideTex_nullData);

			//// below only supported in 4.5
			//glClearTexImage(s_flagTex, 0, GL_RED_INTEGER, GL_INT, &clearColor);
